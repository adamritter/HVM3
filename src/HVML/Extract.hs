  ...
  DH0 -> do
    ...
    then do
      ...
      then do
        ...
        modify $ \x -> (IS.insert (fromIntegral loc) dups, snd x)
        val0 <- extractCore book val
        return $ Duh lab dh0 dh1 val0 (Var dh0)
    else extractCore book sub
  ...
  DH1 -> do
    ...
    then do
      ...
      then do
        ...
        modify $ \x -> (IS.insert (fromIntegral loc) dups, snd x)
        val0 <- extractCore book val
        return $ Duh lab dh0 dh1 val0 (Var dh1)
    else extractCore book sub
...
liftDups (Suh lab tm0 tm1) = do
  tm0 <- liftDups tm0
  tm1 <- liftDups tm1
  return $ Suh lab tm0 tm1
liftDups (Duh lab dp0 dp1 val bod) = do
  val <- liftDups val
  bod <- liftDups bod
  modify (\oldState k -> oldState (Duh lab dp0 dp1 val k))
  return bod
